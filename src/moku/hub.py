...
- `model.onnx`: what Kaya runs (ONNX Runtime Web). Input `pixel_values` `(batch, 3, 640, 640)`,
  ...
  (apply a sigmoid) and `pred_boxes` `(batch, 300, 4)`, normalized `(cx, cy, w, h)`.

## Evaluation

{metrics}

Board metrics run Kaya's own post-processing (stone threshold {threshold}) and compare the
resulting position with the one read from the annotations: *perfect* is the share of boards
without a single wrong intersection. Intervals are 90% bootstrap CIs over photos.
"""
...
def model_card(
    ...
    return CARD_TEMPLATE.format(
        name=name, metrics=metrics_markdown, base_model=base_model, dataset=dataset, threshold=threshold
    )
